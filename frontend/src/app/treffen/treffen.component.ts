...
@Component({
  selector: 'app-treffen',
  templateUrl: './treffen.component.html',
  styleUrls: ['./treffen.component.css']
})
export class TreffenComponent implements OnInit {
  events: Event[] = [];
  requests: Request[] = [];
  ...
  loadEvents(): void {
    this.eventService.getEvents().subscribe({
      next: (events) => {
        this.events = events.filter(event => event.title === 'Treffen');
