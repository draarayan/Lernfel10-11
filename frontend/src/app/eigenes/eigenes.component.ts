...
@Component({
  selector: 'app-eigenes',
  templateUrl: './eigenes.component.html',
  styleUrls: ['./eigenes.component.css']
})
export class EigenesComponent implements OnInit {
  events: Event[] = [];
  requests: Request[] = [];
  ...
  loadEvents(): void {
    this.eventService.getEvents().subscribe({
      next: (events) => {
        this.events = events.filter(event => event.title === 'Eigenes');
