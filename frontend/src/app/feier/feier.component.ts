...
@Component({
  selector: 'app-feier',
  templateUrl: './feier.component.html',
  styleUrls: ['./feier.component.css']
})
export class FeierComponent implements OnInit {
  events: Event[] = [];
  requests: Request[] = [];
  ...
  loadEvents(): void {
    this.eventService.getEvents().subscribe({
      next: (events) => {
        ...
        this.events = events.filter(event => event.title === 'Feier');
